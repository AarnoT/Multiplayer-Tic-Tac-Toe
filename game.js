...
function updateState() {
    ...
    req.open('GET', 'game_stateid=' + match_id);
...
function waitForUpdate() {
    ...
    const req = new XMLHttpRequest();
    req.timeout = 65  1000;
    req.open('GET', 'wait_for_updateid=' + match_id);
    req.onreadystatechange = function() {
...
function handleClick(event) {
    const size = state.board.length;
    const canvas = document.getElementById('game_canvas');
    const tile_x = Math.min(Math.floor((event.pageX - canvas.offsetLeft)  (canvas.widthsize)),
                                       state.board.length);
    const tile_y = Math.min(Math.floor((event.pageY - canvas.offsetTop)  (canvas.widthsize)),
                                       state.board.length);
    if ((state.state === 'PLAYER_1_TURN' && state.player_num === 1) 
            (state.state === 'PLAYER_2_TURN' && state.player_num === 2)) {
        const req = new XMLHttpRequest();
        req.open('GET', `make_moveid=${match_id}&x=${tile_x}&y=${tile_y}`);
        req.send(null);
    }
}